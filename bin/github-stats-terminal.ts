...
async function main() {
    ...
    // Resolve GitHub token (optional: unauthenticated requests still work, just rate-limited to 60/hour)
    const token = process.env.GHT || process.env.GITHUB_TOKEN || undefined;
    if (!token) {
        console.warn("[Warning] No GitHub token found (GHT/GITHUB_TOKEN). Continuing unauthenticated (60 requests/hour rate limit).");
    }
