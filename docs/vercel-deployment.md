...
1. **`api/stats.ts`**: The public, self-contained entrypoint used for actual README embeds. It intercepts query options (`&theme=`, `&headerStyle=`, `&mock=`) and maps them to our TS generator engine, resolving live data **and** rendering in a single request.
...
### Split endpoints behind the Visual Configurator

The Configurator (`index.html`) doesn't call `api/stats.ts` directly while you tweak settings — that would spend a real GitHub API request on every theme change. Instead it uses two internal endpoints:

- **`api/resolve.ts`** (`GET`) — resolves live GitHub stats *only* (no rendering) and returns them as JSON. This is the only one of the three endpoints that spends GitHub API quota. The Configurator caches the response in `localStorage` for an hour.
- **`api/render.ts`** (`POST`) — takes the already-resolved stats (plus your theme/header/art choices) in the request body and renders the SVG, without ever calling GitHub. Switching themes, window styles, or ASCII art re-renders instantly from the cache.

So in the Configurator: changing the **username** spends a GitHub API request (fresh data needed); changing **anything else** doesn't. The final embed URL you copy still points at `api/stats.ts`, which stays fully self-contained for use in an actual README.
