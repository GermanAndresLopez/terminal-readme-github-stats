...
export class TerminalService {
    ...
    constructor(config: TerminalConfig) {
        this.options = config;
        this.theme = typeof config.theme === "string" ? (THEMES[config.theme] || THEMES.dracula) : config.theme;
    }
     ...
     * @returns The zero-based terminal row and recording-timeline second at which this
     *          command's output begins (used e.g. to overlay a real photo onto the
     *          neofetch logo slot, timed to appear in sync with the typing animation).
     */
    public addCommand(commandText: string, outputData: GraphicsDataContract, outputType: TerminalGraphicsType = "text"): { startRow: number; startTime: number } {
        ...
        this.record("\r\n");
        this.wait(0.5);
        const outputStartRow = this.lineCount;
        const outputStartTime = this.currentTime;
        const output: string = this.prepareFormattedOutput(outputData, outputType);
        this.recordOutput(output);
        this.wait(1.0);
        return { startRow: outputStartRow, startTime: outputStartTime };
    }

    /**
     * Returns the current position on the recording timeline (in seconds).
     * Called after all commands have run to learn the total animation duration.
     */
    ...
    /**
     * Converts the resolved Theme into the format required by SvgTerm.
     * @returns A compatible theme object for SvgTerm.
     */
    private getThemeObject(): any {
        const rawTheme: Theme = this.theme;
